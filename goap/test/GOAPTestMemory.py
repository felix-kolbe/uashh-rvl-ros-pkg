...
class TestIncrementer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.memory.set_value('counter', 0)

        print self.memory

        Condition._conditions_dict.clear()

        Condition.add('counter', MemoryCondition('counter', self.worldstate))
        ...
        self.goal = Goal([Precondition(Condition.get('counter'), 3)])

        self.goal_inaccessible = Goal([Precondition(Condition.get('counter'), -2)])
